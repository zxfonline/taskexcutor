...
func main() {
	...
	for i := 0; i < num; i++ {
		go func(i int) {
			...
			ev := SysTimer.AddTimerEvent(taskexcutor.NewTaskService(obj.t1.Processing), fmt.Sprintf("event1_%d", i+1), 0, time.Duration((100+int(randInt(1, 50))*i)*1e6), num+1-i, true)
			obj.t1.event = ev
		}(i)
	}
